...
def backprop(labels, theta, z, h, g, x):
    ...
    for i in range(N):
        ...
        p_h = (f2_W * p_z[:, np.newaxis]).sum(axis=0) 
        p_f2_W = p_f2_W + p_z[:, np.newaxis]
        p_f2_W = p_f2_W * h[i]
        p_f2_b += p_z
...
def update_theta(theta, nabla_J, epsilon):
    ...
    for i in range(len(theta)):
        updated_t = theta[i] - (nabla_J[i] * epsilon)
        updated_theta.append(updated_t)

    return tuple(updated_theta)


# ToDo: set numpy random seed to the last 8 digits of your CWID
np.random.seed(20324562)
...
# hyperparameters
bound = 100 # initial weight range
epsilon = 0.000000001 # learning rate
batch_size = 4
